...
function findTargGen(dist,initNode,distMat){
	var distsOfAllNodesFromInitNode = distMat[initNode];
	var nodesWithWantedDist =[];
	var j,nNodesWithWantedDist=0; // nNodesWithWantedDist is the number of all nodes with the wanted distance  from initNode
	for(j=0;j<G.nNodes;j++){ // j loops over all nodes
		if(distsOfAllNodesFromInitNode[j]==dist){ // if the distance of node j from initNode is dist
			nodesWithWantedDist.push(j); // nodesWithWantedDist is all nodes with the wanted distance (dS) from inP
			nNodesWithWantedDist = nNodesWithWantedDist+1; // count how many elements in nodesWithWantedDist
		}
	}
	var indChosenNodeInArrayOfNodesWithWantedDist = Math.floor( nNodesWithWantedDist * Math.random()); // i
	if(indChosenNodeInArrayOfNodesWithWantedDist==nNodesWithWantedDist){
		indChosenNodeInArrayOfNodesWithWantedDist=nNodesWithWantedDist-1;
	}
	var chosenNode = nodesWithWantedDist[indChosenNodeInArrayOfNodesWithWantedDist];
	return chosenNode;
}
